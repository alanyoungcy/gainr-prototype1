...
class _SwapScreenState extends ConsumerState<SwapScreen> {
  ...
  SwapResult get _swapResult {
    ...
    return ref
        ...
        .calculateSwap(_fromToken, _toToken, amount);
  }
  ...
  Future<void> _handleSwap(double fromAmount, double toAmount) async {
    ...
    if (completed == true && mounted) {
      ...
      await ref
          .read(walletProvider.notifier)
          .swap(_fromToken, _toToken, fromAmount, toAmount);
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _SwapDetailRow(
                  label: 'Price Impact',
                  value: '${(result.priceImpact * 100).toStringAsFixed(2)}%',
                  valueColor: result.priceImpact > 0.01
                      ? Colors.orange
                      : AppTheme.gainrGreen,
                ),
  ...
  double _getTokenBalance(dynamic wallet, String token) {
    ...
    if (token == 'BET') return wallet.betBalance;
          ...
          child: Column(
            ...
            children: [
              ...
              _SwapDetailRow(
                  ...
                  value: '${swapResult.fee.toStringAsFixed(4)} $toToken'),
              _SwapDetailRow(
                label: 'Price Impact',
                value: '${(swapResult.priceImpact * 100).toStringAsFixed(2)}%',
                valueColor: swapResult.priceImpact > 0.01
                    ? Colors.orange
                    : AppTheme.gainrGreen,
              ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                Text(
                  'You swapped $fromAmount $fromToken for ${toAmount.toStringAsFixed(2)} $toToken.',
                  textAlign: TextAlign.center,
                  style: TextStyle(
        ...
        return Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              _TokenRow(
                  name: 'USDC', desc: 'USD Coin', onSelect: onTokenSelect),
              _TokenRow(
                  name: 'BET',
                  desc: 'BACK.bet Utility',
                  onSelect: onTokenSelect),
              _TokenRow(
                  name: 'GAINR',
                  desc: 'GAINR Governance',
                  onSelect: onTokenSelect),
            ],
          ),
        );
...
class _TokenRow extends StatelessWidget {
  ...
  const _TokenRow(
      {required this.name, required this.desc, required this.onSelect});
  ...
  Widget build(BuildContext context) {
    return HoverScaleGlow(
      ...
      child: ListTile(
        contentPadding: EdgeInsets.zero,
        leading: _TokenIcon(token: name, size: 32),
        title: Text(name,
            style: const TextStyle(
...
class _TokenIcon extends StatelessWidget {
  ...
  const _TokenIcon({required this.token, this.size = 20});

  @override
  Widget build(BuildContext context) {
    Color color = Colors.blue;
    if (token == 'BET') color = Colors.orange;
    if (token == 'GAINR') color = AppTheme.gainrGreen;
    ...
    return Container(
      ...
      child: Center(
        child: Text(
          token[0],
          style: TextStyle(
              color: color, fontSize: size * 0.6, fontWeight: FontWeight.bold),
        ),
      ),
    );
  }
