# GAINR Protocol — GOALS v4: Price.bet Transformation Roadmap
...
Transform the forked `DecentraPredict` scaffold into **Price.bet** — a high-performance, compliance-first prediction market dApp powered by GAINR Protocol's System-2 AI Intelligence on Solana.

> Price.bet is **dApp #2** within the full GAINR ecosystem (Back.bet · Price.bet · Pick.bet).
...
| Stage | Task | Status |
|:---|:---|:---|
| **S0B.1** | Implement Solana wallet signature verification middleware on **ALL** POST endpoints (incl. `referral/claim`, `oracle`) | 🔴 Needs Fix — Applied to Market only; unprotected `claimReward` is a **critical fund-theft vector** |
...
| Stage | Task | Status |
|:---|:---|:---|
| **S1.1** | Apply `NonTransferable` + `DefaultAccountState::Frozen` extensions to $GBET mint in `init_bet_mint.rs` | 🟢 Done |
...
| **S1.4** | Fix `betting.rs` — update to `transfer_checked` CPI for Token-2022 (currently causes mismatch error) | 🟢 Done |
| **S1.5** | Fix `sell_shares.rs` — update to `transfer_checked` CPI for Token-2022 | 🟢 Done |
| **S1.6** | Fix `token_mint.rs` for Token-2022 compatibility (mint_to CPI) | 🟢 Done |
| **S1.7** | **Arithmetic Audit**: Replace `f64` with fixed-point arithmetic in all instructions | 🔴 Needs Fix |
...
| Stage | Task | Status |
|:---|:---|:---|
| **S2.1** | CPMM `x * y = k` swap logic in `betting.rs` (lines 202–265) | 🟡 Partial — math present but `net_amount` / `total_collateral_added` mixing is inconsistent |
| **S2.2** | Cash Out: `sell_shares.rs` CPMM inverse | 🟡 Partial — uses `f64` sqrt (line 113), should use integer sqrt for determinism |
...
| **S2.5** | Add `checked_` arithmetic everywhere + replace `as u64` casts with `try_into()` | 🟢 Done |
...
| **S7.5** | **Failsafe Oracle**: Replace `.unwrap()` with proper error handling in `get_oracle_res.rs` | 🔴 Needs Fix |
...
|:---|:---|:---|:---|:---|:---|:---|
| T0 — Foundation | P0A, P0B | 19 | 13 | 0 | 5 | 1 |
| T1 — Core | P1, P2 | 15 | 6 | 3 | 2 | 4 |
| T2 — Compliance/AI | P3, P4 | 14 | 0 | 3 | 1 | 10 |
| T3 — Arch/UX | P5, P6 | 14 | 0 | 0 | 0 | 14 |
| T4 — Security/DevOps/Perf | P7, P8, P9 | 24 | 0 | 0 | 1 | 23 |
| T5 — Ecosystem | P10–P14 | 21 | 0 | 0 | 0 | 21 |
| **TOTAL** | **14 Phases** | **107 Stages** | **19** | **7** | **9** | **72** |

**True Completion: ~6% verified-done, ~13% in progress, ~8% broken/needs-fix**
...
| Token Standard | Token-2022 NonTransferable | ✅ Mint created via `Token2022` | 🟡 Need `NonTransferable` extension |
...
*Version: GOALS v4.0 | Post-audit, source-verified | 2026-02-22*
